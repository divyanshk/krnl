...
Convention:
    The input file should define at minimum:
        - A function named `*_kernel` decorated with @cute.kernel (device code)
        - A function named `*_launch` or `*_wrapper` decorated with @cute.jit (host launcher)
...
class KernelInfo:
    ...
    # The @cute.jit host launcher that compiles and calls the kernel
...
def parse_kernel_file(filepath: Path) -> KernelInfo:
    ...
    launcher_fn: tuple[str, str] | None = None
    ...
    for node in ast.iter_child_nodes(tree):
        ...
        elif fn_name.endswith(("_launch", "_wrapper")):
            launcher_fn = (fn_name, fn_source)
        else:
            # Fallback: if it calls a @cute.kernel function, treat as launcher
            if _calls_any(node, [k[0] for k in kernel_fns]):
                launcher_fn = (fn_name, fn_source)
            else:
    ...
    return KernelInfo(
        ...
        launcher_fn_name=launcher_fn[0] if launcher_fn else None,
        launcher_source=launcher_fn[1] if launcher_fn else None,
...
def _has_cute_kernel_decorator(node: ast.FunctionDef) -> bool:
    """Return True if the function is decorated with @cute.kernel (device-side code)."""
...
def _has_cute_jit_decorator(node: ast.FunctionDef) -> bool:
    """Return True if the function is decorated with @cute.jit (host-side launcher)."""
    for dec in node.decorator_list:
        if (
            isinstance(dec, ast.Attribute)
            and isinstance(dec.value, ast.Name)
...
def _calls_any(node: ast.FunctionDef, fn_names: list[str]) -> bool:
    """Check if a function body calls any of the given function names."""
    for child in ast.walk(node):
        if isinstance(child, ast.Call):
            func = child.func
            if isinstance(func, ast.Subscript) and isinstance(func.value, ast.Name):
                if func.value.id in fn_names:
                    return True
...
def reconstruct_kernel_file(
    ...
    new_launcher_source: str | None = None,
) -> str:
    """Reconstruct a full runnable file with a new kernel implementation.

    Keeps the preamble, reference function, and test inputs from the original.
    Replaces the @cute.kernel function(s) and optionally the @cute.jit launcher.
    """
    parts = [info.preamble, "", new_kernel_source, ""]

    if new_launcher_source:
        parts.append(new_launcher_source)
    elif info.launcher_source:
